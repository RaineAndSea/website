...
export const SidePanel: FC<{closeSidePanel: () => void, sidePanelToggleButton: React.MutableRefObject<HTMLDivElement | null>}> = ({closeSidePanel, sidePanelToggleButton}) => {
    const panelRef = useRef<HTMLDivElement | null>(null);
    
    useEffect(() => {
      const handleOutsideClick = (event: MouseEvent) => {
        const isSidePanel = panelRef.current && panelRef.current.contains(event.target as Node);
        const isSidePanelToggleButton = sidePanelToggleButton.current && sidePanelToggleButton.current.contains(event.target as Node);
