...
export const Login: FC<{isRegistering: boolean, setIsRegistering: (isRegistering: boolean) => void}> = ({isRegistering, setIsRegistering}) => {
    ...
    const handleSubmit = (vals: LoginBody) => {
        ...
        toast.promise(promise, authToastMessages)
        .then(res => {
            ...
            <Formik initialValues={{email: '', password: ''}} onSubmit={(vals: LoginBody) => handleSubmit(vals)}>
                {(props) => (
                    <Form className={loginForm}>
                        ...
                        <button className={regLoginSubmitButton} type="submit">Login</button>
                        <p className={regLoginHyperlink} onClick={() => setIsRegistering(!isRegistering)}>Need an account? <span className="decorated">Register here</span></p>
                    </Form>
                )}
            </Formik>
        </div>
