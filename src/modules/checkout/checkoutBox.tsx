        ...
        <>
             <section style={{width: '100%', maxHeight: '90%', overflowY: 'scroll'}}>
                    {Object.keys(cart.products).map((id, key) => (
                        <CheckoutCartItem key={key} productId={id} qty={cart.products[id as keyof object]} setCart={(cart) => cartUpdateFn(cart)} isLast={key === Object.keys(cart.products).length - 1} />
                    ))}
                </section>
                {!mobile && (
                    <>
